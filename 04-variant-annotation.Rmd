...
```{r echo=FALSE}
paged_table(head(dplyr::select(geno_novo_not_ref_high_qual, Location, Allele, mother, father, proband, counter)))
```
...
```{r echo=FALSE}
paged_table(head(dplyr::select(prob_novo_high_qual,SYMBOL,Uploaded_variation,HGVSc, HGVSp, mother, father, proband, counter)))
```
...
```{r gnomAD_hom, message=FALSE, warning=FALSE}
library(ghql)
library(jsonlite)
...
paged_table(head(dplyr::select(prob_hom, SYMBOL, Uploaded_variation, HGVSc, HGVSp, gnomADe_hom, gnomADg_hom, proband, counter)))
```
...
```{r echo=FALSE, warning=FALSE}
paged_table(head(dplyr::select(ungroup(prob_comp_het2),SYMBOL,Uploaded_variation,HGVSc,HGVSp,proband, counter, PS_proband)))
```
...
```{r gnomAD_comp_het, message=FALSE}
...
```{r echo=FALSE}
paged_table(dplyr::select(ungroup(dplyr::filter(prob_2plus_het2,SYMBOL=="HLA-DQA1")),SYMBOL,Uploaded_variation,HGVSc,HGVSp,proband,PS_proband,gnomAD_A,gnomAD_B, gnomAD_PS, gnomAD_cis, gnomAD_trans))
```
...
```{r echo=FALSE,warning=FALSE}
paged_table(head(dplyr::select(ungroup(prob_X), SYMBOL, Uploaded_variation,HGVSc,HGVSp,mother,father,proband, counter)))
```
...
```{r echo=FALSE,warning=FALSE}
paged_table(head(dplyr::select(refseq_au,SYMBOL,Uploaded_variation,HGVSg,HGVSc,Status)))
```
